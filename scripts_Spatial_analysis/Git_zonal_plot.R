...
{
  corr_mean_df = data.frame("AABI" = input_AABI_glc_all_mean_de_scaled_df$input_AABI_glc_all_mean_de_scaled, "TRENDY" = gpp_TRENDY_combined_de_scaled_df$mean,"FLUXCOM" = input_gpp_FLUXCOM_de_scaled_df$input_gpp_FLUXCOM_de_scaled,"RS" = input_gpp_RS_mean_de_scaled_df$input_gpp_RS_mean_de_scaled)
  corr = cor(corr_mean_df)
  p.mat = cor_pmat(corr_mean_df)
  
  pdf(".\\Spatial_analysis\\Fig1_P31.pdf",width = 9,height = 6)
...
#plot code for North America Fig1f
{
    pdf(".\\Spatial_analysis\\Fig1_P32.pdf",width = 12,height = 5.5)
    p32 <- ggplot()+
      geom_ribbon(data = gpp_TRENDY_combined_de_scaled_df,aes(x=Year,ymin= min,
                                                              ymax= max),fill = "green4",alpha = 0.2)+
      geom_line(data = input_AABI_glc_all_mean_de_scaled_df,aes(x=Year,
                                                            y=input_AABI_glc_all_mean_de_scaled,color = "aa"),linetype="solid",size =1.5)+
      ...
      geom_line(data = gpp_TRENDY_combined_de_scaled_df,aes(x=Year,y= mean,color = "dd"),linetype="solid",size = 1.5)+
      ...
      ylab("z-score")+
      xlab("Year")+
      scale_y_continuous(limits = c(-5.9,4),breaks= c(-4,-2,0,2,4),labels = sprintf("%.1f", c(-4,-2,0,2,4)))+
      #scale_fill_identity(name = 'the fill', guide = 'legend',labels = c('m1')) #+
      scale_color_manual(name = '',
                         values =c("aa"="gray21","bb" = "royalblue","cc"="lightcoral","dd"="green4"), labels = c(bquote("AABI"[per_area]),bquote("GPP"[FLUXCOM]),bquote("GPP"[RS_mean]),bquote("GPP"[TRENDY])))+
      theme_set(theme_bw())+
...
#Plot code for Global ED Fig1d
{
  ...
  pdf("D:\\MEGA\\Git\\Growth_GPP\\Spatial_analysis\\FigS1_d.pdf",width = 10,height =6)
  p32 <- ggplot()+
